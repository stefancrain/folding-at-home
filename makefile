...
buildx: ## build locally
	make pretty
	make lint
	docker buildx build --load --platform linux/amd64 --build-arg=VERSION=7.6.13 --build-arg=VERSION_MAJOR=7.6 --build-arg=VCS_REF=1 --build-arg=BUILD_DATE=1 --tag "stefancrain/folding-at-home:local" -f ./build/Dockerfile ./build/
